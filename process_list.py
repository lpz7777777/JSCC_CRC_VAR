...
def get_compton_backproj_list(list_origin, compton_arg, detector, coor_polar, sysmat, device):
    ...
    # get_scatter_angle
    ee = 0.511
    theta = torch.acos(1 - ((ee * e1) / ((compton_arg.e0 - e1) * compton_arg.e0)))
    # calculate the angular error contributed by energy
    er = e1 * compton_arg.ene_resolution / 2.355 * (compton_arg.e0 / e1) ** 0.5
    angle_sigma_ene = er * (1 / torch.abs(torch.sin(theta))) * ee / (compton_arg.e0 - e1) ** 2
    klein_nishina = compton_arg.e0 / (compton_arg.e0 - e1) + (compton_arg.e0 - e1) / compton_arg.e0

    # get_angle_sigma
    p = distance01 / distance12
    q = compton_arg.delta_r1 / compton_arg.delta_r2
    angle_sigma_pos = torch.atan(compton_arg.delta_r1 / distance01) * (1 + p ** 2 * (1 + q ** 2) + 2 * p * torch.cos(theta.unsqueeze(-1))) ** 0.5
